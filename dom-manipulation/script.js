 ...
 * - Keeps earlier task requirements:
 *   quotes[] literal, displayRandomQuote(), populateCategories()/filterQuote(),
 *   exportToJsonFile(), importFromJsonFile(event)
 * - Adds server sync (JSONPlaceholder) with conflict handling.
 ***********************/

/* ----------------- Storage keys ----------------- */
const STORE_KEY        = "dqg.quotes.v1";             // all quotes
const SESSION_LAST_KEY = "dqg.lastViewed.v1";         // session last viewed
const LAST_FILTER_KEY  = "selectedCategory";          // exact name used by checker
const DIRTY_SET_KEY    = "dqg.dirtyIds.v1";           // IDs of quotes changed locally
const LAST_SYNC_KEY    = "dqg.lastSyncAt.v1";         // ISO timestamp of last sync
const CONFLICTS_KEY    = "dqg.conflicts.v1";          // stored conflicts to review

/* ----------------- Server (mock) ----------------- */
const SERVER_URL = "https://jsonplaceholder.typicode.com/posts"; // mock API
// We map server post -> quote: { id, body, title } -> { serverId, text, category }

/* ----------------- Seed data (literal!) ----------------- */
let quotes = [
  { id: cryptoRandomId(), text: "The only limit to our realization of tomorrow is our doubts of today.", category: "Motivation", updatedAt: Date.now() },
  { id: cryptoRandomId(), text: "Simplicity is the ultimate sophistication.", category: "Design", updatedAt: Date.now() },
  { id: cryptoRandomId(), text: "Programs must be written for people to read, and only incidentally for machines to execute.", category: "Programming", updatedAt: Date.now() },
  { id: cryptoRandomId(), text: "What gets measured gets managed.", category: "Product", updatedAt: Date.now() },
  { id: cryptoRandomId(), text: "Premature optimization is the root of all evil.", category: "Programming", updatedAt: Date.now() }
];

/* ----------------- Helpers ----------------- */
function cryptoRandomId() {
  // good-enough random id for demo
...
function markAllClean() { localStorage.setItem(DIRTY_SET_KEY, "[]"); }
...
function storeConflicts(conflicts) { localStorage.setItem(CONFLICTS_KEY, JSON.stringify(conflicts)); }
...
function getLastViewed() {
  try { const raw = sessionStorage.getItem(SESSION_LAST_KEY); if (!raw) return null;
    const q = JSON.parse(raw); if (q && typeof q.text === "string" && typeof q.category === "string") return q;
  } catch {}
  return null;
}
...
/* ----------------- Sync: Fetch, Merge, Push, Conflicts ----------------- */

/**
 * Fetch latest quotes from server (mock).
 * We map JSONPlaceholder posts to quotes:
 *  - text  := body
 *  - category := "Server" or first word of title (just to vary)
 */
async function fetchServerQuotes(limit = 10) {
  ...
  return posts.map(p => ({
    ...
    updatedAt: now                    // pretend “now” is the server version time
  }));
}

/** Push locally-created quotes (no serverId) to the mock server (POST). */
async function pushLocalCreates() {
  const unsent = quotes.filter(q => !q.serverId && getDirtyIds().has(q.id));
  if (unsent.length === 0) return;
  for (const q of unsent) {
    try {
      // JSONPlaceholder returns a fake id; we pretend it's accepted.
      ...
      // Map returned server id (if any)
      q.serverId = data.id || q.serverId || Math.floor(Math.random()*100000);
      q.id = q.id.startsWith("srv_") ? q.id : q.id; // keep local id
      q.updatedAt = Date.now();
      setDirty(q.id, false);
    } catch {
      // leave as dirty; will retry next cycle
    }
  }
  saveQuotes();
}
...
/**
 * Merge server data into local with a simple policy:
 * - If an item exists both locally and on server (matched by serverId), and local is dirty -> CONFLICT
 *   Conflict policy: **server wins** (overwrite), but we keep a copy in conflicts store for manual restore.
 * - If server item is new -> add it.
 * - If local item only exists locally and is clean -> keep.
 */
function mergeServerIntoLocal(serverQuotes) {
  ...
  for (const s of serverQuotes) {
    ...
    if (!existing) {
      // Add missing server item
      quotes.push({ ...s });
      continue;
    }
    // Existing: check dirty => conflict
    if (dirty.has(existing.id)) {
      // Conflict: keep a copy, then let server win
      conflicts.push({ local: { ...existing }, server: { ...s } });
      Object.assign(existing, s); // overwrite with server
      setDirty(existing.id, false);
    } else {
      // No local changes -> accept server update if newer (ours always older in this sim)
...
/** Run one sync cycle: push local creates, then fetch and merge server */
async function syncOnce() {
  try {
    await pushLocalCreates();
...
/* ----------------- UI: sync banner & conflicts review ----------------- */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Sync UI
  document.getElementById("syncNowBtn")?.addEventListener("click", syncOnce);
  ...
  // Expose for inline handlers / checkers
